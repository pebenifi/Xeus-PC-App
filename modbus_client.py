        ...
        try:
            ...
            for actual_framer in framers_to_try:
                try:
                    ...
                    # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Восстановлены настройки из рабочей версии от 27 ноября
                    # Проблема: коммит 63c7494 (14 дек) уменьшил timeout с 3s до 0.05s, что сломало Modbus
                    # Устройство не успевает отвечать за 0.05 секунды
                    self.client = ModbusTcpClient(
                        host=self.host, 
                        port=self.port, 
                        framer=actual_framer,
                        timeout=3.0,  # 3 секунды - как в рабочей версии от 27 ноября (было 0.05s - сломало работу)
                        retries=1,    # 1 повтор для надежности (было 0 - сломало работу)

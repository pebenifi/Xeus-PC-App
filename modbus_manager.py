...
# Laser PSU ток (1231) и setpoint (1241): устройство хранит A × 10 (152 → 15.2 A)
_LASER_PSU_AMPS_SCALE = 10.0


def _laser_psu_register_to_amps(raw: object) -> Optional[float]:
    try:
        return float(int(raw)) / _LASER_PSU_AMPS_SCALE
    except Exception:
        return None


def _laser_psu_amps_to_register(amps: float) -> int:
...
def _laser_temp_register_to_celsius(raw: object) -> Optional[float]:
    try:
        return float(int(raw)) / _LASER_TEMP_SCALE
    except Exception:
        return None
...
class ModbusManager(QObject):
    ...
    clinicalForegroundChanged = Signal(bool)
    ...
    _workerEnqueueReadPriority = Signal(str, object)  # для IR/NMR — в начало очереди
    ...
    def __init__(self, parent=None):
        ...
        # Время последнего возобновления опроса (для игнорирования проверки соединения сразу после возобновления)
        ...
        self._workerEnqueueReadPriority.connect(self._io_worker.enqueueReadPriority)
    ...
    def _begin_priority_register_read(
        self,
        key: str,
        flag_attr: str,
    ...
    def setClinicalForeground(self, active: bool) -> None:
        ...
        self.clinicalForegroundChanged.emit(active)
    ...
    def disableValvePolling(self):
        ...
        # ВРЕМЕННО: Не выключаем опрос клапанов
        pass
    ...
    def disableFanPolling(self):
        ...
        # ВРЕМЕННО: Не выключаем опрос вентиляторов
        pass
    ...
    @Property(bool, notify=clinicalForegroundChanged)
    def clinicalForeground(self):
        """Clinical на переднем плане — Screen01 не должен дублировать IR/NMR."""
        return bool(self._clinical_foreground)
    ...
    @Property(float, notify=laserTempChanged)
    def laserTemp(self):
        """Температура лазера (регистр 1841)"""
    ...
    def disconnect(self):
        ...
        try:
            ...
            self._laser_temp_timer.stop()  # Останавливаем чтение температуры лазера
    ...
    def _onWorkerConnectFinished(self, success: bool, error_message: str):
        ...
        if not success:
            ...
            if self._reconnect_polling_stopped:
                self._reconnect_polling_stopped = False
    ...
    def _onWorkerReadFinished(self, key: str, value: object):
        ...
        if key == "ir":
            payload = self._io_worker._last_spectrum.pop("ir", None)
            self._ir_request_in_flight = False
            if payload is None:
                logger.info("IR spectrum read returned None")
            else:
                logger.info("IR spectrum read completed, applying to graph")
                self._last_modbus_ok_time = time.time()
                self._connection_fail_count = 0
            self._applyIrSpectrum(payload)
            return

        if key == "nmr":
            payload = self._io_worker._last_spectrum.pop("nmr", None)
            self._nmr_request_in_flight = False
            if payload is None:
        ...
        if key == "1021":
            ...
            self._applyLaserPSURegisters(value)
        elif key == "1841":
            self._applyLaserTempValue(value)
        elif key == "1611":
    ...
    def _onWorkerWriteFinished(self, key: str, success: bool, meta: object):
        ...
        if success:
            ...
            if key.startswith("relay:") or key.startswith("fan:") or key.startswith("valve:"):
                ...
                # Обновляем время последней записи, чтобы логика чтения отработала корректно
                self._last_write_time = time.time()
                self._last_write_key = key
    ...
    def _enqueue_read_priority(self, key: str, func: Callable[[], Any]) -> None:
        """Поставить задачу чтения в начало очереди (IR/NMR спектры)."""
        try:
            self._workerEnqueueReadPriority.emit(key, func)
        except Exception:
            logger.exception("Failed to enqueue priority read task")
    ...
    def _applyWaterChillerSetpointValue(self, value: object):
        ...
        logger.info(f"✅ [1531] Water Chiller Setpoint обновлен из устройства: {setpoint}°C")
    ...
    def _applySeopCellTemperatureValue(self, value: object):
        ...
        try:
            ...
            # Исправлено масштабирование: делим на 10.0, а не на 100.0 (как PID controller)
            # Если 236 -> 23.6, значит делитель 10
    ...
    def _apply_setpoint_from_register_1421(self, setpoint: float) -> None:
        """Регистр 1421 общий для SEOP Cell и PID Controller setpoint."""
        updated = False
        if not self._seop_cell_setpoint_user_interaction and self._seop_cell_setpoint != setpoint:
            self._seop_cell_setpoint = setpoint
            self.seopCellSetpointChanged.emit(setpoint)
            updated = True
        if not self._pid_controller_setpoint_user_interaction and self._pid_controller_setpoint != setpoint:
            self._pid_controller_setpoint = setpoint
            self.pidControllerSetpointChanged.emit(setpoint)
            updated = True
        if updated:
            logger.info(f"✅ [1421] Setpoint обновлен из устройства: {setpoint}°C")
    ...
    def _applySeopCellSetpointValue(self, value: object):
        ...
        self._apply_setpoint_from_register_1421(setpoint)
    ...
    def _applyLaserPSURegisters(self, value: object):
        ...
        if current is not None and self._laser_psu_current != current:
            ...
            logger.debug(f"✅ [1231] Laser PSU Current: {current} A")

        if not self._laser_psu_setpoint_user_interaction:
            current_sp = _laser_psu_register_to_amps(value.get("1241"))
            if current_sp is not None and self._laser_psu_setpoint != current_sp:
                self._laser_psu_setpoint = current_sp
                self.laserPSUSetpointChanged.emit(current_sp)
                logger.debug(f"✅ [1241] Laser PSU Current Setpoint: {current_sp} A")

        if "1251" in value:
            try:
                driver_on = bool(int(value["1251"]) & 0x01)
            except Exception:
                driver_on = None
    ...
    def _applyN2SetpointValue(self, value: object):
        ...
        if self._n2_setpoint != setpoint:
            ...
            logger.info(f"✅ [1661] N2 Setpoint обновлен из устройства: {setpoint} Torr (применено напрямую)")
    ...
    def _applyIrSpectrum(self, value: object):
        ...
        if not value or not isinstance(value, dict):
            logger.info("IR spectrum: empty/invalid payload (not a dict or None)")
            return
    ...
    def requestIrSpectrum(self) -> bool:
        """
        IR spectrum as in driver ReadReg (controller_wifi.cpp):
        - 400: pointsN (typically ~356, not 58)
        - 401-414: float metadata
        - data is NOT 58 consecutive registers at 420.
          j = (baseAddr - 420) * 64 + (addr - baseAddr),
          so stripe k is FC04 start=420+k, qty<=64 → points[k*64 : k*64+qty].
        """
        if not self._is_connected or self._modbus_client is None:
        ...
        def task():
            import math
            import struct
            # Читаем 400..414 (метаданные) одним блоком — иначе иногда "плывут" поля.
            meta = client.read_input_registers(400, 15)
            if meta is None or len(meta) < 15:
                logger.info(f"IR spectrum: meta read failed or short: {None if meta is None else len(meta)}")
                return None

            # pymodbus, не прямой сокет: read_input_registers_direct давал SIGSEGV на живой шине.
            # Драйвер: IR_CHART_ARRAYSIZE=64, IR_CHART_ARRAYS=58. Один запрос с 420 даёт только
            # первые min(qty,64) точек; остальные — отдельные FC04 со start=421, 422, ...
            status = int(meta[0])
            ir_chart_data = 420
            ir_chart_arraysize = 64
            ir_chart_arrays = 58
            n_points = status
            max_points = ir_chart_arrays * ir_chart_arraysize
            if n_points < 2:
                logger.info(f"IR spectrum: pointsN={n_points}, skip")
                return None
            if n_points > max_points:
                logger.info(f"IR spectrum: pointsN={n_points} capped to {max_points}")
                n_points = max_points

            n_stripes = (n_points + ir_chart_arraysize - 1) // ir_chart_arraysize
            data_regs: list[int] = [0] * n_points
            for k in range(n_stripes):
                start_idx = k * ir_chart_arraysize
                qty = min(ir_chart_arraysize, n_points - start_idx)
                stripe = client.read_input_registers(ir_chart_data + k, qty)
                if stripe is None or len(stripe) < qty:
                    logger.info(
                        f"IR spectrum: stripe {k} addr={ir_chart_data + k} qty={qty} "
                        f"failed or short: {None if stripe is None else len(stripe)}"
                    )
                    return None
                data_regs[start_idx:start_idx + qty] = [int(v) for v in stripe[:qty]]

            if sum(1 for v in data_regs if v != 0) == 0:
                logger.info("IR spectrum: data all zeros, skip apply")
                return None

            logger.info(
                f"IR spectrum: pointsN={n_points} stripes={n_stripes} "
                f"raw meta[0..4]={meta[0:5]} meta_hex={[hex(int(x)) for x in meta[0:5]]} "
            ...
            # y values (raw uint16 from device) — все pointsN точек, как aseq->points[]
            y_values_raw_u16 = [int(v) for v in data_regs[:n_points]]
            ...
            # Все pointsN точек растягиваем на [x_min, x_max]
            points = []
            ...
            if y_values:
                ...
                if math.isfinite(y_min_data) and math.isfinite(y_max_data):
                    ...
                    y_min = 0.0
                    y_max = 1.0
                    logger.debug("IR spectrum: processed data range invalid, using fallback")
            else:
                y_min = 0.0
                y_max = 1.0
            ...
            logger.debug(
                ...
                f"y_axis=[{y_min:.6f},{y_max:.6f}] (autoscale from processed data) "
                f"y_processed_range=[{y_min_processed:.6f},{y_max_processed:.6f}] "
                f"y_min_meta={y_min_meta_str} y_max_meta={y_max_meta_str} "
                f"baseline={m:.6f} "
                f"points={len(points)} (pointsN={n_points}) raw_u16_range=[{min(y_values_raw_u16) if y_values_raw_u16 else 'n/a'},{max(y_values_raw_u16) if y_values_raw_u16 else 'n/a'}] "
            ...
            if len(result["data"]) != n_points:
                logger.error(f"IR spectrum: result.data length mismatch: {len(result['data'])} != {n_points}")
            if len(result["points"]) != n_points:
                logger.error(f"IR spectrum: result.points length mismatch: {len(result['points'])} != {n_points}")
            try:
                data_json_parsed = json.loads(result["data_json"])
                if len(data_json_parsed) != n_points:
                    logger.error(f"IR spectrum: result.data_json length mismatch: {len(data_json_parsed)} != {n_points}")
            except Exception as e:
    ...
    def requestNmrSpectrum(self) -> bool:
        """
        NMR spectrum as in driver ReadReg (controller_wifi.cpp / modbus.h):
        - 100: samplesN (typically ~330; LCD uses ptMax+1-ptMin)
        - 101-116: float metadata (x_min/x_max/y_min/y_max/freq/ampl/integral/t2)
        - data is NOT 256 consecutive uint16 at 120.
          Each FFT sample is a float (2 uint16). NMR_CHART_ARRAYSIZE=64,
          NMR_CHART_ARRAYS=256. Stripe k is FC04 start=120+k, qty<=64:
          j = (baseAddr - 120) * 64 + (addr - baseAddr),
          FFT[j>>1] words are u.w[j&1] (STM32 little-endian = Modbus CDAB).
        pymodbus read_input_registers only — not *_direct (SIGSEGV).
        """
        if not self._is_connected or self._modbus_client is None:
        ...
        def task():
            ...
            def _float_from_regs_cdab(reg1: int, reg2: int) -> float:
                # STM32 u.w[0], u.w[1] → IEEE big-endian bytes CDAB
                C = (reg2 >> 8) & 0xFF
                D = reg2 & 0xFF
                A = (reg1 >> 8) & 0xFF
                B = reg1 & 0xFF
            ...
            # 100..116 (17 regs) — consecutive switch(addr), not striped
            meta = client.read_input_registers(100, 17)
            if meta is None or len(meta) < 17:
                logger.info(f"NMR spectrum: meta read failed or short: {None if meta is None else len(meta)}")
                return None

            nmr_chart_data = 120
            nmr_chart_arraysize = 64
            nmr_chart_arrays = 256
            n_points = int(meta[0])
            max_points = (nmr_chart_arrays * nmr_chart_arraysize) // 2
            if n_points < 2:
                logger.info(f"NMR spectrum: samplesN={n_points}, skip")
                return None
            if n_points > max_points:
                logger.info(f"NMR spectrum: samplesN={n_points} capped to {max_points}")
                n_points = max_points

            n_regs = n_points * 2
            n_stripes = (n_regs + nmr_chart_arraysize - 1) // nmr_chart_arraysize
            data_regs: list[int] = [0] * n_regs
            for k in range(n_stripes):
                start_idx = k * nmr_chart_arraysize
                qty = min(nmr_chart_arraysize, n_regs - start_idx)
                stripe = client.read_input_registers(nmr_chart_data + k, qty)
                if stripe is None or len(stripe) < qty:
                    logger.info(
                        f"NMR spectrum: stripe {k} addr={nmr_chart_data + k} qty={qty} "
                        f"failed or short: {None if stripe is None else len(stripe)}"
                    )
                    return None
                data_regs[start_idx:start_idx + qty] = [int(v) for v in stripe[:qty]]
            ...
            data_values = [
                _float_from_regs_cdab(data_regs[i], data_regs[i + 1])
                for i in range(0, n_regs, 2)
            ]
            finite_y = [v for v in data_values if math.isfinite(v)]
            if not finite_y or all(v == 0.0 for v in finite_y):
                logger.info("NMR spectrum: data all zeros/invalid, skip apply")
                return None


            x_min = _float_from_regs_cdab(int(meta[1]), int(meta[2]))
            x_max = _float_from_regs_cdab(int(meta[3]), int(meta[4]))
            y_min_meta = _float_from_regs_cdab(int(meta[5]), int(meta[6]))
            y_max_meta = _float_from_regs_cdab(int(meta[7]), int(meta[8]))
            ...
            logger.info(
                f"NMR spectrum: samplesN={n_points} stripes={n_stripes} "
                f"x=[{x_min},{x_max}] freq={freq} ampl={ampl} "
                f"data_first5={data_values[:5]} data_last5={data_values[-5:]}"
            )

            n = len(data_values)
            if not (math.isfinite(x_min) and math.isfinite(x_max) and x_max > x_min):
                # Driver X is freqStep*ptMin .. freqStep*(ptMin+n-1); keep a display fallback.
                logger.info(f"NMR spectrum: invalid x range [{x_min},{x_max}], using 38000..44000")
                x_min = 38000.0
                x_max = 44000.0

            y_data_min = float(min(finite_y))
            y_data_max = float(max(finite_y))
            # LCD PaintNMR: ymin=0, ymax=resultAmpl. Autoscale from data; keep 0 floor when non-negative.
            y_axis_min = 0.0 if y_data_min >= 0.0 else y_data_min
            y_axis_max = y_data_max
            if math.isfinite(ampl) and ampl > y_axis_max:
                y_axis_max = float(ampl)
            ...
            points = []
            if n >= 2:
                for i, y in enumerate(data_values):
                    yi = float(y) if math.isfinite(y) else 0.0
                    x = x_min + (x_max - x_min) * float(i) / float(n - 1)
                    points.append({"x": x, "y": yi})
            else:
                for i, y in enumerate(data_values):
                    yi = float(y) if math.isfinite(y) else 0.0
                    points.append({"x": float(i), "y": yi})
            ...
            result = {
                ...
                "data": [float(v) if math.isfinite(v) else 0.0 for v in data_values],
                "data_json": json.dumps([float(v) if math.isfinite(v) else 0.0 for v in data_values]),
                "points": points,
            }
            logger.info(
                f"NMR spectrum: returning payload with {len(result['data'])} data points, "
                f"{len(result['points'])} graph points"
            )
            return result
    ...
    def _check_connection(self):
        ...
        # Не проверяем соединение в течение 15 секунд после подключения/возобновления опроса
        # Это дает время на первые чтения регистров и стабилизацию соединения
        if self._polling_resumed_time > 0 and (now - self._polling_resumed_time) < 15.0:
            return
        ...
        self._reconnect_polling_stopped = True
        try:
            for t in self._polling_timers:
                t.stop()
        except Exception:
            pass
        for t in (
            self._water_chiller_setpoint_auto_update_timer,
            self._magnet_psu_setpoint_auto_update_timer,
            self._laser_psu_setpoint_auto_update_timer,
            self._seop_cell_setpoint_auto_update_timer,
            self._xenon_setpoint_auto_update_timer,
            self._n2_setpoint_auto_update_timer,
        ):
            try:
                t.stop()
            except Exception:
    ...
    def _autoUpdateLaserPSUSetpoint(self):
        """Сбрасывает блокировку ввода и снова читает setpoint с устройства."""
        if self._laser_psu_setpoint_user_interaction:
            self._laser_psu_setpoint_user_interaction = False
            logger.debug("Laser PSU setpoint: сброс флага ввода, читаем значение с устройства")
        self._readLaserPSURegisters()

    def _autoUpdateWaterChillerSetpoint(self):
        """Сбрасывает блокировку ввода и читает setpoint (1531) с устройства."""
        if self._water_chiller_setpoint_user_interaction:
            self._water_chiller_setpoint_user_interaction = False
        self._readWaterChillerSetpoint()

    def _applyLaserTempValue(self, value: object):
        """Температура лазера (регистр 1841)."""
        self._reading_laser_temp = False
        if value is None:
            return
        temp = _laser_temp_register_to_celsius(value)
        if temp is None:
            return
        self._laser_temp = temp
        self.laserTempChanged.emit(temp)
        logger.debug(f"✅ [1841] Laser Temp: {temp}°C")
    ...
    def _readLaserPSURegisters(self):
        ...
        def read_laser_psu():
            ...
            if all(v is None for v in regs.values()):
                return None
            return {k: (v if v is not None else 0) for k, v in regs.items()}

        self._enqueue_read("laser_psu", read_laser_psu)
    ...
    def _autoUpdateSeopCellSetpoint(self):
        """Сбрасывает блокировку ввода и читает setpoint (1421) с устройства."""
        if self._seop_cell_setpoint_user_interaction:
            self._seop_cell_setpoint_user_interaction = False
        if self._pid_controller_setpoint_user_interaction:
            self._pid_controller_setpoint_user_interaction = False
        self._readSeopCellSetpoint()

    def _readSeopCellSetpoint(self):
        """
        Чтение регистра 1421 (setpoint SEOP Cell / PID Controller) из устройства
        """
        if not self._is_connected or self._modbus_client is None:
            return
    ...
    def _readN2Setpoint(self):
        """
        Чтение регистра 1661 (setpoint N2) из устройства
        """
        if not self._is_connected or self._modbus_client is None:
            return
        ...
        client = self._modbus_client
        # Используем обычный pymodbus
        # Регистр 1661 - Holding Register (записываемый)
        self._enqueue_read("1661", lambda: client.read_holding_register(1661))
    ...
    def _applyPendingUIUpdates(self):
        ...
        # КРИТИЧНО: если идет запись ИЛИ прошло мало времени после нее, НЕ применяем значения из кэша
        # Это предотвращает применение старых значений во время записи и сразу после нее
        # Прямое обновление (через _readStateAfterWrite -> _apply...Value) работает в обход этого кэша
        time_since_write = current_time - self._last_write_time
        if self._write_in_progress:
            # logger.debug(f"⏸️ Пропускаем применение значений из кэша (запись)")
            return
        ...
        for relay_name, (new_state, timestamp) in self._pending_relay_updates.items():
            ...
            if hasattr(self, '_last_write_key') and self._last_write_key.startswith("relay:") and time_since_write < 1.0:
        ...
        for fan_index, (new_state, timestamp) in self._pending_fan_updates.items():
            ...
            # КРИТИЧНО: не применяем значения, которые были прочитаны ДО начала записи реле или вентилятора
            if hasattr(self, '_last_write_key') and (self._last_write_key.startswith("relay:") or self._last_write_key.startswith("fan:")) and time_since_write < 1.0:
                if timestamp < self._last_write_time:
                    # logger.debug(f"⏭️ [UI] Пропускаем применение вентилятора {fan_index} (значение прочитано ДО записи)")
                    to_remove_fan.append(fan_index)
                    continue
                # Также игнорируем все вентиляторы сразу после записи
                # logger.debug(f"⏭️ [UI] Пропускаем применение вентилятора {fan_index} (произошло после записи)")
                to_remove_fan.append(fan_index)
                continue
    ...
    def _readPIDController(self):
        ...
        def task():
            ...
            # Регистр 1431 - on/off (1 = вкл, 0 = выкл); setpoint 1421 читается отдельным таймером
    ...
    def _readLaser(self):
        ...
        def task():
            ...
            if temp_value is not None:
                temp = _laser_temp_register_to_celsius(temp_value)
                if temp is not None:
                    result['temp'] = temp
    ...
    def _setFanAsync(self, fanIndex: int, fan_bit: int, state: bool):
        ...
        self._enqueue_write(f"fan:{fanIndex}", task, {"fanIndex": fanIndex, "state": state})
    ...
    def _write_water_chiller_setpoint_register(self, register_value: int, temperature: float) -> bool:
        ...
        if result:
            ...
            logger.error(f"❌ Не удалось записать setpoint Water Chiller: {temperature}°C (1531={register_value})")
        return bool(result)
    ...
    def setWaterChillerTemperature(self, temperature: float) -> bool:
        ...
        logger.info(f"🔵 setWaterChillerTemperature: {temperature}°C")
        ...
        def task() -> bool:
            return self._write_water_chiller_setpoint_register(register_value, temperature)

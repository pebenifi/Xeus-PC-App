...
class ModbusManager(QObject):
    ...
    def _applyRelay1021Value(self, value: object):
        ...
        # Удаляем старые записи ожидаемых состояний (старше 5 секунд - увеличили таймаут)
        self._expected_states = {k: v for k, v in self._expected_states.items() 
                                if not k.startswith('relay:') or current_time - v[1] < 5.0}
        
        # Игнорируем чтение в течение 500мс после удаления ожидаемого состояния
        # (увеличили задержку, чтобы не применять временные неправильные значения из регистра)
        if current_time - self._last_expected_state_removed_time_1021 < 0.5:
        ...
        # и НЕ применяем НИКАКИЕ значения из регистра, пока все ожидаемые состояния не будут удалены
        if relay_expected:
            ...
            # ВАЖНО: НЕ применяем НИКАКИЕ значения из регистра, только удаляем из ожидаемых при совпадении
            relay_key_map = {
            ...
            for relay_name, new_state in new_states.items():
                ...
                if expected_info is not None:
                    ...
                    if new_state == expected_state:
                        ...
                        # НЕ применяем значение из регистра - оно уже в памяти (оптимистичное обновление)
            return  # Полностью игнорируем чтение, пока есть ожидаемые состояния
    ...
    def _applyValve1111Value(self, value: object):
        ...
        current_time = time.time()
        # Удаляем старые записи ожидаемых состояний для клапанов (старше 5 секунд)
        self._expected_states = {k: v for k, v in self._expected_states.items() 
                                if not k.startswith('valve:') or current_time - v[1] < 5.0}
        
        # Игнорируем чтение в течение 500мс после удаления ожидаемого состояния
        if current_time - self._last_expected_state_removed_time_1111 < 0.5:
        ...
        # Если есть ожидаемые состояния, полностью игнорируем чтение
        if valve_expected:
    ...
    def _applyFan1131Value(self, value: object):
        ...
        current_time = time.time()
        # Удаляем старые записи ожидаемых состояний для вентиляторов (старше 5 секунд)
        self._expected_states = {k: v for k, v in self._expected_states.items() 
                                if not k.startswith('fan:') or current_time - v[1] < 5.0}
        
        # Игнорируем чтение в течение 500мс после удаления ожидаемого состояния
        if current_time - self._last_expected_state_removed_time_1131 < 0.5:
